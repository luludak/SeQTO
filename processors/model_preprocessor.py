...
class ModelPreprocessor:
    ...
    def preprocess(self, model_name, img_data):
        ...
        if ("disable_preprocessing" in library_or_name):
        ...
        if("torch" in library_or_name):
            ...
            transforms.CenterCrop(self.data["input_dimension"][0]),
            transforms.ToTensor(),
        ...
        if("onnx" in library_or_name):
            transform = transforms.Compose([
            transforms.Resize(224),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
    
            img_tensor = np.expand_dims(transform(img_data), axis=0)
        ...
        elif ("imagenet" in library_or_name):
        ...
        if("googlenet" in library_or_name):
            preprocessed_value = self.preprocess_googlenet(img_data)
    
        elif ("densenet" in library_or_name):
            
            # Note: this is the suggested preprocessing for densenet etc.
            preprocessed_value = vgg_preprocess_image(np.array(img_data), img_data.shape[2], img_data.shape[2])
            return np.expand_dims(np.array(preprocessed_value), axis=0)

        elif ("mobilenet" in library_or_name or "shufflenet" in library_or_name):
            preprocessed_value = self.preprocess_imagenet(preprocessed_value)
    
        elif("inceptionv3" in library_or_name or "inception_v3" in library_or_name or "resnet" in library_or_name):
